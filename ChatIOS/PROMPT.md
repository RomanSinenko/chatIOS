...
- нижний input как overlay над чатом:
  - input находится поверх области сообщений, как остров
  - сообщения могут уходить под input
  - нижняя область под input размывается/material до нижней границы экрана
  - кнопка отправки появляется внутри input только при наличии текста
- стабильный scroll-contract чата:
  - одинаковая нижняя позиция при первом входе, автоскролле вниз и ручном скролле вниз
...
# Текущий фокус
Текущий большой iOS-шаг: `ios-chat-input-websocket-send`.

Цель шага:
- привести экран чата к устойчивому layout перед подключением отправки
- закрепить компактный title в верхней части экрана без большого navigation title
- оставить input как overlay/остров поверх чата, с нижним blur/material слоем
- после layout подключить отправку сообщений через backend `WS /ws`
- обработать `message_ack`
- добавить live-входящие сообщения без выхода/возврата в чат
